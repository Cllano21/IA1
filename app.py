...
# Manejo robusto de CSV y fechas mixtas
df = pd.read_csv(csv_path, on_bad_lines='skip')  # Ignora filas corruptas
df["trip_completed_at"] = pd.to_datetime(
    df["trip_completed_at"], errors='coerce', infer_datetime_format=True
)

# Eliminar filas con fechas inválidas
df = df.dropna(subset=["trip_completed_at"])

# Convertimos a dólares (ya que la columna tiene "cents")
df["boost_dollars"] = df["cumulative_boost_amount_cents"] / 100  # opcional: mostrar en dólares reales
...
def update_chart(selected_markets):
    if not selected_markets:
        return px.line(title="No market selected")
    ...
    fig.update_layout(
        yaxis_title="Total Boost ($)",
        xaxis_title="Date",
        hovermode="x unified"  # Mejor hover para ver todos los mercados en la misma fecha
    )
    return fig
...
if __name__ == "__main__":
    ...
    # debug=False en Render para evitar reinicios innecesarios
